...
async function searchMultipleQueries(queries, videosPerQuery = 1) {
  const searchPromises = queries.map((queryObj) => {
    ...
    const difficulty =
      typeof queryObj === "string" ? "intermediate" : queryObj.difficulty;
...
async function verifyVideosRelevance(model, originalText, keyConcepts, videos) {
  ...
  const videoList = videosWithDetails
    .map((v, i) => `${i}: "${v.title}" - ${v.description.slice(0, 150)}`)
    .join("\n");
    ...
    if (Array.isArray(data.search_queries) && data.search_queries.length > 0) {
      ...
      if (videos.length > 0) {
        if (SKIP_VERIFICATION) {
          ...
          const verifiedVideos = await verifyVideosRelevance(
            model,
            text,
            data.key_concepts,
            videos,
